...
-- RSVP write trigger: maintains live, daily, and monthly aggregates
...
begin
  if tg_op = 'INSERT' then
    v_event_id := new.event_id; v_status := coalesce(new.status, 'attending'); v_paid := coalesce(new.paid,false); v_amount := coalesce(new.amount_paid_cents,0);
    ...
    v_event_id := new.event_id; v_status := coalesce(new.status, 'attending'); v_paid := coalesce(new.paid,false); v_amount := coalesce(new.amount_paid_cents,0);
    -- derive deltas comparing OLD→NEW
    ...
    -- total only changes on insert/delete; keep 0 here
  elsif tg_op = 'DELETE' then
    v_event_id := old.event_id; v_status := coalesce(old.status, 'attending'); v_paid := coalesce(old.paid,false); v_amount := coalesce(old.amount_paid_cents,0);
    delta_total := -1; if v_status = 'attending' then delta_attend := -1; elsif v_status = 'cancelled' then delta_cancel := -1; end if; if v_paid then delta_paid := -1; end if;
  end if;
