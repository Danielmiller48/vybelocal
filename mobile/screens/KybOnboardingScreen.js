...
export default function KybOnboardingScreen() {
  ...
  const submitIndividual = async () => {
    try {
      ...
      // Hold base (non-sensitive) legal entity; proceed to secure Step 4 form
      setBaseLegal(legal_entity);
      // Prefill bank holder from legal name if empty
      if (!bankHolder) setBankHolder(legal_entity?.legal_name || profile?.name || user?.email || 'VybeLocal User');
      // Initialize idempotency key for final submit
  ...
  const finalizeIndividual = async () => {
    try {
      ...
      merged.bank_account = {
        ...
        routing_number: String(bankRouting).replace(/\D/g,''),
        account_number: String(bankAccount).replace(/\s/g,''),
        account_holder_name: bankHolder || merged.legal_name || null,
      };
      ...
      const res = await fetch(`${API_BASE_URL}/api/payments/tilled/onboarding`, {
        ...
        body: JSON.stringify({ legal_entity: merged, tos_acceptance: true })
      });
      const text = await res.text();
      // Show payload back in dev for verification
      if (__DEV__) {
        try { console.log('[KYB][client] submitted legal_entity:', JSON.stringify(merged, null, 2)); } catch {}
      }
          ...
          {step === 4 && bizType === 'sp' && (
            <Animated.View style={{ opacity: fade, transform: [{ translateX: slide.interpolate({ inputRange:[-1,0,1], outputRange:[-24,0,24] }) }] , marginTop:0, marginBottom:24, backgroundColor:'#fff', borderRadius:16, padding:16, borderWidth:1, borderColor:'#E0E7FF' }}>
              ...
              <View style={{ marginBottom:8 }}>
                ...
                <TextInput value={bankHolder} onChangeText={setBankHolder} placeholder="Full name" style={{ borderWidth:1, borderColor:'#E5E7EB', borderRadius:10, paddingHorizontal:12, paddingVertical:10 }} />
              </View>
              <View style={{ marginBottom:8 }}>
              ...
              <View style={{ flexDirection:'row', justifyContent:'space-between', marginTop:16 }}>
                ...
                <TouchableOpacity disabled={isSubmitting} onPress={finalizeIndividual} style={{ width:'48%', backgroundColor: isSubmitting ? '#6EE7B7' : '#10B981', borderRadius:12, paddingVertical:12, alignItems:'center' }}>
