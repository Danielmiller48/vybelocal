...
// Our EventCard has a 16-px margin on each side. Keep crop frame identical.
const SIDE_MARGIN = 16;
const CARD_W = SCREEN_W - SIDE_MARGIN * 2; // match EventCard width
const SCREEN_HALF_W = SCREEN_W / 2;
const SCREEN_HALF_H = SCREEN_H / 2;
const CARD_H = 200;
const FRAME_LEFT = SIDE_MARGIN;
const FRAME_RIGHT = SCREEN_W - SIDE_MARGIN;
...
export default function ImageCropModal({ uri, visible, onClose, onCrop }){
  ...
  useEffect(()=>{
    ...
    RNImage.getSize(uri,(w,h)=>{
      ...
      imgW.value = w;
      imgH.value = h;
      // Ensure the image covers the entire crop frame (CARD_W × CARD_H)
      const coverScale = Math.max(CARD_W / w, CARD_H / h);
  ...
  const clampWorklet = (tx, ty, scl)=>{
    ...
    // Constrain so the image always covers the CARD frame horizontally
    const minX = FRAME_RIGHT - halfW - SCREEN_HALF_W; // align right edge to frame right
    const maxX = FRAME_LEFT  + halfW - SCREEN_HALF_W; // align left edge to frame left
  ...
  async function handleCrop(){
    ...
    const cropW = CARD_W * (imgW.value / renderW);
      ...
      <GestureHandlerRootView style={styles.container}>
        ...
        <PinchGestureHandler onGestureEvent={pinchHandler}>
          <Animated.View style={{ flex:1 }}>
            ...
            <View style={[styles.frameBorder, { width: CARD_W, left: FRAME_LEFT }]} pointerEvents="none" />
          </Animated.View>
        </PinchGestureHandler>
...
const styles = StyleSheet.create({
  ...
  frameBorder:{ position:'absolute', height:CARD_H, top: FRAME_TOP, borderWidth:2, borderColor:'#fff' },
}); 