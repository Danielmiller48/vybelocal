...
begin
  -- Cleanup any previous seed artifacts for this host
  ...
  delete from public.events where host_id = v_host and (title like 'SEED:%' or title like 'Seeded Event %');

  -- Seed 12 months of test events (8-12 per month)
  for m in 0..11 loop
    for e in 1..(8 + (random()*4)::int) loop
      v_event_id := gen_random_uuid();
      v_event_start := (date_trunc('month', now()) - (m||' months')::interval) 
                       + ((random()*27)::int || ' days')::interval 
                       + ((18 + (random()*4)::int) || ' hours')::interval; -- 6-10pm
      v_title := 'SEED: Event #'||e||' (M-'||m||')';
      v_desc := 'Seeded analytics test event';
      select (array['chill','hype','creative','active'])[1 + floor(random()*4)]::text into v_vibe;
      v_price := case when random() < 0.8 then (1500 + (random()*4000)::int) else 0 end; -- 80% paid
      v_capacity := 100 + (random()*400)::int; -- 100-500
      v_rsvps := greatest(20, least(v_capacity, (v_capacity * (0.45 + random()*0.55))::int)); -- 45-100%
      v_attending := (v_rsvps * (0.88 + random()*0.08))::int; -- 88-96%
      v_cancelled := v_rsvps - v_attending;
      v_gross := case when v_price > 0 then v_rsvps * v_price else 0 end;
      v_platform_rate := 0.02 + random()*0.02;   -- 2% - 4%
      ...
      -- Insert base event (flagged as seed)
      insert into public.events(id, host_id, title, description, vibe, starts_at, rsvp_capacity, price_in_cents, created_at)
      values (v_event_id, v_host, v_title, v_desc, v_vibe, v_event_start, v_capacity, v_price, v_first_rsvp - interval '1 day');
      ...
      -- Insert event_daily distribution
      ...
      while day_cursor <= v_last_rsvp::date and rsvps_remaining > 0 loop
        ...
        v_daily := greatest(1,
          case
            ...
            when v_days_until <= 3 then (v_rsvps * 0.14 * (0.5 + random()))::int
            when v_days_until <= 7 then (v_rsvps * 0.08 * (0.5 + random()))::int
        ...
        insert into analytics.event_daily(
          ...
          v_daily,
          (v_daily * (v_attending::numeric / nullif(v_rsvps,0)))::int,
          (v_daily * (v_cancelled::numeric / nullif(v_rsvps,0)))::int,
  ...
  -- Roll up to host_monthly (last 12 months present even if zero)
  with months as (
